...
static u8 mrf24j40_set_channel(u8 channel)
{
    ...
    g_mrf24j40_ctx.channel = channel;
    return 0;
}
...
static int mrf24j40_read_channel_assessment(struct rf_mac_channel_assessment* arg){
    int ret =100;
    mrf24j40_set_short_reg(MRF24J40_WRITE_BBREG6, 0x80);
    arg->noise_level = mrf24j40_get_short_reg(MRF24J40_READ_BBREG6);
    while(((arg->noise_level & 0x01) != 0x01) && (ret > 0)){
        arg->noise_level = mrf24j40_get_short_reg(MRF24J40_READ_BBREG6);
        mdelay(1);
        ret -= 1;
    }
    arg->noise_level = mrf24j40_get_long_reg(0x210);
...
//			spin_unlock(&g_mrf24j40_ctx.lock);
		...
		case RF_MAC_IOC_WR_CHANNEL:{
		    unsigned int* ch = (unsigned int*)arg;
//		    spin_lock(&g_mrf24j40_ctx.lock);
...
//		    spin_unlock(&g_mrf24j40_ctx.lock);
		...
		case RF_MAC_IOC_RD_CHANNEL:{
		    unsigned int* ch = (unsigned int*)arg;
		    *ch = g_mrf24j40_ctx.channel;
		    ret = 0;
		    break;
		}
		case RF_MAC_IOC_RD_SHORT_ADDRESS:{
		    u8* addr = (u8*)arg;
		    addr[0] = g_mrf24j40_ctx.short_address[0];
		    addr[1] = g_mrf24j40_ctx.short_address[1];
		    ret = 0;
		    break;
		}
        case RF_MAC_IOC_RD_LONG_ADDRESS:{
            u8* addr = (u8*)arg;
            int i;
            for(i = 0; i < 8 ; i++)
                addr[i] = g_mrf24j40_ctx.long_address[i];
            ret = 0;
            break;
        }
        case RF_MAC_IOC_RD_PAN_ID:{
            u8* pan = (u8*)arg;
            pan[0] = g_mrf24j40_ctx.pan_id[0];
            pan[1] = g_mrf24j40_ctx.pan_id[1];
            ret = 0;
            break;
        }
		case RF_MAC_IOC_RD_CHANNEL_ASSESSMENT:{
//		    spin_lock(&g_mrf24j40_ctx.lock);
		    ret = mrf24j40_read_channel_assessment((struct rf_mac_channel_assessment*)arg);
//		    spin_unlock(&g_mrf24j40_ctx.lock);
		    break;
		}
		case RF_MAC_IOC_WR_RESET:{
		    int i;
		    spin_lock(&g_mrf24j40_ctx.lock);
		    for(i = 0; i < DRV_MRF24J40_WRITE_COUNT; i++){
		        g_mrf24j40_ctx.write_list[i].flags = 0x00;
